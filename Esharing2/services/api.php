<?php
   ...
   */
   ...
   public function processApi(){
     ...
     private function Posts_Houses_byType(){
       if($this->get_request_method() != "GET"){
      $this->response('',406);
     }
     $query="SELECT * FROM post, house WHERE post.ItemType='house' AND post.HouseID=house.HouseID";
     $r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
     ...
     private function House(){
       ...
       $id = (int)$this->_request['id'];
       if($id > 0){
         $query="SELECT * FROM house WHERE house.HouseID=$id";
         $r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
         if($r->num_rows > 0) {
           $result = $r->fetch_assoc();
           $this->response($this->json($result), 200); // send user details
     ...
     private function updateCar(){
      ...
      private function updateHouse(){
         ...
         $House = json_decode(file_get_contents("php://input"),true);
         $id = (int)$Car['id'];
         $column_names = array('Rent', 'Sqft', 'Rooms', 'Baths');
         $keys = array_keys($House['House']);
         $columns = '';
         $values = '';
         foreach($column_names as $desired_key){ // Check the customer received. If key does not exist, insert blank into the array.
            if(!in_array($desired_key, $keys)) {
         ...
         $$desired_key =$House['House'][$desired_key];
         }
         $columns = $columns.$desired_key."='".$$desired_key."',";
         }
         $query = "UPDATE house SET ".trim($columns,',')." WHERE HouseID=$id";
         if(!empty($House)){
         $r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
         $success = array('status' => "Success", "msg" => "House".$id." Updated Successfully.", "data" => $House);
         $this->response($this->json($success),200);
         }else
         $this->response('',204); // "No Content" status
       }
      ...
      private function insertHouse_Post(){
        if($this->get_request_method() != "POST"){
        $this->response('',406);
        }
        $Item = json_decode(file_get_contents("php://input"),true);
        $column_names = array('Rent', 'Sqft', 'Rooms', 'Baths');
        $keys = array_keys($Item['House']);
        $columns = '';
        $values = '';
        foreach($column_names as $desired_key){ // Check the user received. If blank insert blank into the array.
           if(!in_array($desired_key, $keys)) {
        ...
        $$desired_key = $Item['House'][$desired_key];
        }
        $columns = $columns.$desired_key.',';
        $values = $values."'".$$desired_key."',";
        }
        $query = "INSERT INTO house(".trim($columns,',').") VALUES(".trim($values,',').")";
        if(!empty($Item['House'])){
        $r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
        $Item['Post']['HouseID']=$this->mysqli->insert_id;
        }
        $column_names = array('DateCreated', 'CreatorID', 'Title', 'ItemType', 'Description', 'AvailStart', 'AvailEnd', 'Address', 'CarID', 'HouseID', 'imgsrc');
        $keys = array_keys($Item['Post']);
        $columns = '';
     ...
     private function Post_House_User(){
         if($this->get_request_method() != "GET"){
   				$this->response('',406);
   			}
   			$id = (int)$this->_request['id'];
   			if($id > 0){
   				$query="SELECT * FROM post, house, user WHERE post.ID=$id AND post.HouseID=house.HouseID AND post.CreatorID=user.ID";
   				$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
   				if($r->num_rows > 0) {
   					$result = $r->fetch_assoc();
   					$this->response($this->json($result), 200); // send user details
     ...
     private function deleteHouse(){
      ...
      $id = (int)$this->_request['id'];
      $HouseID=(int)$this->_request['HouseID'];
      if($id > 0){
   ...
   private function sendMail(){
     ...
     $column_names = array('SendID', 'RecID', 'TimeSent', 'Message', 'Sender', 'Receiver');
     $keys = array_keys($Mail);
     $columns = '';
     $values = '';
     foreach($column_names as $desired_key){ // Check the user received. If blank insert blank into the array.
        if(!in_array($desired_key, $keys)) {
     ...
     $$desired_key = $Mail[$desired_key];
     }
     $columns = $columns.$desired_key.',';
     $values = $values."'".$$desired_key."',";
     }
     $query = "INSERT INTO mail(".trim($columns,',').") VALUES(".trim($values,',').")";
     if(!empty($Mail)){
     $r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
     $success = array('status' => "Success", "msg" => "message inserted Successfully.", "data" => $User);
     $this->response($this->json($success),200);
     }else
     $this->response('',204); //"No Content" status
   }
