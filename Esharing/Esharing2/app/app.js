...
app.factory('sessionService',['$http',function ($http,$window) {
  return {
    ...
    destroy:function (key) {
      $http.post('services/destroy');
      return sessionStorage.removeItem(key);
    }
  };
}]);

app.factory("services", ['$http','sessionService','$location', function($http, sessionService, $location) {
    ...
    obj.loginverify = function(email, password,scope) {
        return $http.get(serviceBase + 'loginverify?email=' + email + '&password=' + password).then(function (msg) {
          var uid=msg.data;
          if(uid){
    ...
    obj.islogged=function () {
      return $http.post(serviceBase+'check_session');
    }
    ...
    obj.getUserInfo=function (UserID) {
      return $http.get(serviceBase + 'getUserInfo?id='+UserID).success(function (results) {
        return results.data;
      });
    };
...
app.controller('postCtrl', function($scope, services, $location, $route,sessionService) {
    var id = parseInt(JSON.parse(sessionService.get("uid"))["uid"]);
...
app.controller('CarEditCtrl', function($scope, services, $routeParams, Car, Post, $location, Upload, sessionService) {
    ...
    $scope.saveCar = function() {
        if (ID <= 0) {;
            ...
            $scope.Post.CreatorID = parseInt(JSON.parse(sessionService.get("uid"))["uid"]);
...
app.controller('CarViewCtrl', function($scope, services, $routeParams, Car, sessionService) {
    $scope.Car = Car.data;
    $scope.sendMail = function() {
      var Mail = {};
      Mail.SendID = parseInt(JSON.parse(sessionService.get("uid"))["uid"]);
      services.getUserInfo(Mail.SendID).then(function (data) {Mail.RecID = Car.data.CreatorID;
        var datenow=new Date();
        Mail.TimeSent = datenow.getTime();
        Mail.Message = $scope.content;
        Mail.Sender = data.data.Fname;
...
app.controller('HouseViewCtrl', function($scope, services, $routeParams, House, sessionService) {
...
app.controller('MailCtrl', function($scope, Messages, services, $location, sessionService) {
  var id=parseInt(JSON.parse(sessionService.get("uid"))["uid"]);
...
app.controller('UserConsole',function ($scope, $location, services, sessionService) {
  $scope.$watch(function () { return sessionService.get("uid") }, function (newVal, oldVal) {
      if (typeof newVal !== "undefined") {
        if(newVal) $scope.email=JSON.parse(newVal)["uid"];
        else $scope.email="login";
      }
  });
    ...
    function($routeProvider) {
        ...
        when('/login', {
            ...
            .when('/mailBox', {
                ...
                controller: 'MailCtrl',
                resolve: {
                  Messages: function (services,sessionService) {
                    var UserID=parseInt(JSON.parse(sessionService.get("uid"))["uid"]);
...
app.run(['$location', '$rootScope','services', function($location, $rootScope,services) {
    var routePermission=['/mailBox','/postManage'];
    $rootScope.$on('$routeChangeStart',function () {
      if(routePermission.indexOf($location.path())!=-1){
        services.islogged().then(function(msg){
